		...
		while url_finish == 0:
			...
			try:
				if (TT <= 1):
					...
					time.sleep(2)
					try:
						element = WebDriverWait(self.driver1, 5 + 3 * TT).until(
							EC.presence_of_element_located((By.CLASS_NAME, "chartselectdiy")))
						url_finish = 1
					except:
						print("!@#!!$!@")
				else:
					...
					time.sleep(2)
					element = WebDriverWait(self.driver1, 5 + 2 * TT).until(
						EC.presence_of_element_located((By.CLASS_NAME, "chartselectdiy")))
